...
use libs\connection\BaseConnection;


class AccountRepository extends BaseConnection
{
    ...
    public function getByDate($fecha_ini, $fecha_fin){
        $result = null;
        try {
            $query ="SELECT * FROM cuentas AS c
            INNER JOIN titular AS t ON t.id = c.id_titular
            WHERE c.fecha_ini = ? AND c.fecha_fin = ?";
            $query = $this->prepare($query);
            $query->bindValue(1, $fecha_ini);
            $query->bindValue(2, $fecha_fin);
            $query->execute();
            $result = $query->fetchAll(PDO::FETCH_ASSOC);
        } catch (PDOException $e) {
            print_r($e);die();
